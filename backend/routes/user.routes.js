...
router.route("/update-pass").patch( verifyJwt,userNewPassword)
router.route("/currentuser").get( verifyJwt,currentUser)
router.route("/update-profile").patch( verifyJwt,updateUserDetail)
router.route("/update-avatar").patch( verifyJwt,upload.single("avatar"), UpdateAvatarFile )
router.route("/update-cover").patch( verifyJwt, upload.single("coverImage"),UpdateCoverFile)
