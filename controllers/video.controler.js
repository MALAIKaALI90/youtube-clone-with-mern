...
const getAllVideos=asyncHandler(async (req,res) => {
  ...
  if (!allVideos) {
  return new ApiError(500,"there is not such route")
  }
  return res.status(200).json(
    new ApiResponse(200,"these are all the video you made",allVideos)
  )
  })
const publishVideo=asyncHandler(async (req,res) => {
    //all written fiels are present
...
return res.status(200).json(
    new ApiResponse(200,videomade,"video is uploaded successfully")
)
})
//get video id
// // Get video by ID
...
const videoDelete = async (req, res) => {
  try {
    ...
    console.log( await Video.findById(req.params.videoId));
